...
The `Limit()` query returns some subset of record IDs / keys from a given [row call](/pql-guide/pql#row-calls){:target="_blank"} based on the limit and offset arguments provided. 

`Limit()` is a [row call](/pql-guide/pql#row-calls){:target="_blank"}.
...
#### Mandatory Arguments
- `ROW_CALL` : the [row call](/pql-guide/pql#row-calls){:target="_blank"} to be limited
