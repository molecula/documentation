...
#### NOTE: 
  - `Apply()` is a query run against the `float64` and `int64` field types. For more information on using these data types in FeatureBase, visit the [ingest documentation](/community/community-data-ingestion/dataframe-consumer) and the [HTTP API](/community/community-api/http-api#dataframe-endpoints) endpoints for them.
