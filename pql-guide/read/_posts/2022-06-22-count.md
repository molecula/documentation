...
The `Count()` query returns the number of record IDs / keys returned by a [row call](/pql-guide/pql#row-calls){:target="_blank"}. Note that this includes the [Distinct()](/pql-guide/read/distinct) call -- see the example below.
