...
The `"CacheConfig"` option specifies the size and type of a [`TopN`](/reference/pql-guide/read/topn) cache for a set or mutex field.
...
When operating on a field without a cache, a slower [`TopK`](/reference/pql-guide/read/topk) or sorted [`GroupBy`](/reference/pql-guide/read/groupby) query may be used instead.
