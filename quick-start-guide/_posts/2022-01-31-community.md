...
Ranking queries are notorious for being computationally intensive - aka slow. Some solutions will use statistics to speed up a ranking query by approximating the true results, but that’s not always a desirable option. In addition to SQL, FeatureBase has a native language called [PQL](/data-querying/pql/introduction). In PQL, [```TopK```](/reference/data-querying-ref/pql/read/topk) queries can be run to return exact results in milliseconds. 
...
Another query commonly seen in aggregation-related use cases is the ```GROUP BY```. For example, let’s group by the hobbies counting only those with ultimate ```COUNT``` above 200,000,000. We'll execute this query in [PQL](/data-querying/pql/introduction).
