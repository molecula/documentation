...
While the deployment is spinning up, you will see messages on the ```Deployments``` screen as the status progresses. The status will show as ```CREATING``` while the deployment is spinning up and ```RUNNING``` when the process is done.
