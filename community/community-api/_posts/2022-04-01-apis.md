...
The gRPC interface is described in detail in [gRPC API](/community/community-api/grpc-api).

FeatureBase ships with a Python client and CLI which can be used to interact with FeatureBase via gRPC. See the [client usage](/community/query-data/libraries/python-library) for API examples.

## HTTP

The HTTP interface is described in some detail in [HTTP API](/community/community-api/http-api). Generally, most HTTP endpoints are not needed for normal operations, with the exception of the transaction interface which is used to pause ingestion to take backups. This is documented below.
...
See [Ingesters](/community/community-data-ingestion/ingesters) for information on data ingestion.
