...
- [`etcd.listen-client-address`](/setting-up-featurebase/enterprise/featurebase-configuration#etcd-listen-client-address) to the node's new network address
- [`etcd.listen-peer-address`](/setting-up-featurebase/enterprise/featurebase-configuration#etcd-listen-peer-address) to the node's new network address
- [`etcd.initial-cluster`](/setting-up-featurebase/enterprise/featurebase-configuration#etcd-initial-cluster) to use the new network addresses and add additional nodes

Additionally, if the replication factor needs to be changed, the [`cluster.replicas`](/setting-up-featurebase/enterprise/featurebase-configuration#cluster-replicas) setting should be updated now.
...
The `featurebase` command line tool contains a [`backup`](/reference/operations/enterprise/backups#featurebase) subcommand for executing a backup against a cluster:
```
featurebase backup --host featurebase:10101 -o /path/to/backup/ --concurrency 4 # and TLS config
```
The [`--concurrency`](/reference/operations/enterprise/backups#backup-concurrency) flag is not required, but setting it to an appropriate number will improve backup speed.

It is possible to speed the process up more by [disabling sync](/reference/operations/enterprise/backups#storage-synchronization) of the backup data:
```
...
The `featurebase` command line tool has an accompanying [restore](/reference/operations/enterprise/backups#featurebase) subcommand that will restore a directory created by the backup subcommand:
```
...
Once the new system is up and running, the [ingester configurations](/reference/data-ingestion/ingester-configuration) can be updated to point to the new cluster (`pilosa-hosts` and `pilosa-grpc-hosts`, respectively `--featurebase-hosts` and `--featurebase-grpc-hosts` with `--future.rename` flag).
