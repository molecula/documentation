...
A [basic introduction to PQL](/data-querying/pql-intro) is also available.
...
`Set` assigns a value of 1 to a bit in the binary matrix, thus associating the given row (the `<ROW>` value) in the given field (the `<FIELD>` value) with the record id (the `<COLUMN>` value). `<COLUMN>` and `<ROW>` can each be specified as an integer ID, or for string-keyed Columns and Rows, as the keyed string value.
...
Set the bit at row 1, for unkeyed record id 10:
```pql
Set(10, stargazer=1)
```
```json
...
Set the bit at row 1, for keyed column "10":
```pql
Set("10", stargazer=1)
```
```json
