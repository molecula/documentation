...
* `trackExistence` (bool): Enables or disables existence tracking on the index. Required for [Not](/data-querying/pql/introduction#not) queries. It is `true` by default.
...
Sends a [query](/data-querying/pql/introduction) to the FeatureBase server with the given index. The request body is UTF-8 encoded text and response body is in JSON by default.
