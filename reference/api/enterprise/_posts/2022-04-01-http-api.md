...
* `trackExistence` (bool): Enables or disables existence tracking on the index. Required for [Not](/reference/data-querying/pql#not) queries. It is `true` by default.
...
Sends a [query](/reference/data-querying/pql) to the FeatureBase server with the given index. The request body is UTF-8 encoded text and response body is in JSON by default.
