...
It contains information about the cluster nodes, data, and has an interface for querying the cluster with both [PQL](/data-querying/pql) and [SQL](/data-querying/sql). Note that the UI uses the [Postgres Wire Protocol](/reference/api/enterprise/apis#postgres-wire-protocol), which requires PQL queries to be prefixed with the table name in square brackets, e.g.:
