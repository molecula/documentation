...
sidebar_label: Sum()
---
`Sum()` computes the sum of values in an Int, Decimal, or Timestamp field in some subset of records (defined by a [row call](/data-querying/pql#row-calls){:target="_blank"}).
...
#### Optional Arguments
 - `ROW_CALL` : the [row call](/data-querying/pql#row-calls){:target="_blank"} used to filter records to get the summation over.
