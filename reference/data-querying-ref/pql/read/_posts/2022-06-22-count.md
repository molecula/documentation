...
The `Count()` query returns the number of record IDs / keys returned by a [row call](/data-querying/pql#row-calls){:target="_blank"}. Note that this includes the [Distinct()](/reference/data-querying-ref/pql/read/distinct) call -- see the example below.
...
```
#### Explanation
[All()](/reference/data-querying-ref/pql/read/all) returns a list of all the record IDs in the index. There are six records so `Count()` returns 6.
