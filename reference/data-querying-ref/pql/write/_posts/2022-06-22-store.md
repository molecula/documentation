...
The `Store()` call can be used populate values in a Set field based on the results of a [row call](/data-querying/pql#row-calls){:target="_blank"}. This can be seen as a way of caching [row calls](/data-querying/pql#row-calls){:target="_blank"}.
...
#### Mandatory Arguments
- `ROW_CALL` : a [rows call](/data-querying/pql#rows-calls){:target="_blank"} used to determine which records to write to -- i.e. the record IDs/keys returned in this row call will have `FIELD_VALUE` in `FIELD`. Records that aren't return by this row call will no longer have FIELD\_VALUE in FIELD if they did previously.
