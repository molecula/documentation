...
## Authentication
When authentication is enabled, only users with admin permissions will be able to perform ingest.
...
### 2. auth-token flag 
A valid JWT must be passed to the `auth-token` flag for any ingester. The token may be obtained by following these [instructions](/how-tos/enable-auth#how-to-get-auth-token). 
