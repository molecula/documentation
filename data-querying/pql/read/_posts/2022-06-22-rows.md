...
`Rows()` returns a list of field values (row IDs / keys) in a field. Most of the time, `Rows()` will be used as the argument of another call (e.g. [Extract()](/data-querying/pql/read/extract) or [GroupBy()](data-querying/pql/read/groupby)) and only the FIELD argument will be supplied. In some cases (i.e. [GroupBy()](data-querying/pql/read/groupby))) it makes sense to use additional arguments as filters to group by a subset of field values. See [GroupBy()](/data-querying/pql/read/groupby) for examples.
