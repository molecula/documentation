...
`UnionRows()` is supplied, as arguments, any number of [Rows()](/data-querying/pql/read/rows) calls. Conceptually, `UnionRows()` does the following:
...
`UnionRows()` is a [row call](/data-querying/pql/introduction#row-calls){:target="_blank"}.
...
#### Mandatory Arguments
- `ROWS_CALL` : a [rows call](/data-querying/pql/introduction#rows-calls){:target="_blank"} (set of record IDs / keys). Note the FIELD argument to `Rows()` here must be a Set, Mutex, or Time field.

#### Optional Arguments
- `...` : Any number of additional [rows calls](/data-querying/pql/introduction#rows-calls){:target="_blank"} seperated by commas. Note the FIELD argument to `Rows()` here must be a Set, Mutex, or Time field.
