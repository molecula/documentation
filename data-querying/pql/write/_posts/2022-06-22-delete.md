...
The `Delete()` call removes entire records. A [row call](/data-querying/pql/introduction#row-calls){:target="_blank"} is used to determine which records will be deleted.
...
#### Mandatory Arguments
- `ROW_CALL` : the [row call](/data-querying/pql/introduction#row-calls){:target="_blank"} used to determine which records to delete -- i.e. records returned by `ROW_CALL` will be deleted.
