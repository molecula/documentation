...
(Added to /sql-preview/data-types/cloud-timestamp-data-type)
...
(Added to /sql-preview/data-types/cloud-bool-data-type)
...
| ------- | ------------ | --------- | --------- |
| MIN, MAX   | INT  | The minimum and maximum integer values expected for the column. Integers need to be between `-2^63` and `2^63 -1` | MIN(`-2^63`)  MAX (`2^63 -1`) <!--UI/API is limited to `-2^53` and `2^53 -1` due to precision loss due to JS and Go integer limitations at scale"--> |
